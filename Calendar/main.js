...
function resetKalendara () {
    ...
    document.getElementById("dani").innerText='';

}

function popuniKalendar (godina, mijesec) {
    let imenaMijeseci = [
        'Januar', 'Februar', 'Mart', 'April', 'Maj', 'Jun', 'Jul', 'Avgust', 'Septembar', 'Oktobar', 'Novembar', 'Decembar'
    ];
    let imeIzabranogMijeseca = imenaMijeseci[mijesec-1];
    let t = imeIzabranogMijeseca + ' ' + godina;
    document.getElementById('title').innerText = t;

    let prvi= new Date(godina, mijesec-1, 1);
    let kraj= new Date(godina, mijesec, 0)
    ...
    popuniTijeloKalendara(nedelje);
}

function popuniTijeloKalendara (nedelje) {
